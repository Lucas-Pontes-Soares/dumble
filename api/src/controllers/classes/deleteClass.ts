...
export const deleteClass = async (req: Request, res: Response) => {
    ...
    if (!userAuthenticated || userAuthenticated.role !== 'teacher') {
        return res.status(403).json({ message: 'Somente professores podem deletar turmas' });
    }
    ...
    try {
        ...
        if (ownerCheck.rows[0].teacher_id !== userAuthenticated.id) {
            return res.status(403).json({ message: 'Você não tem permissão para deletar esta turma' });
        }

        const result = await db.query('DELETE FROM classes WHERE id = $1 RETURNING *', [class_id]);
        return res.status(200).json({ message: 'Turma deletada', class: result.rows[0] });
